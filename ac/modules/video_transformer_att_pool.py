...
class VideoTransformerAttPool(nn.Module):

    def __init__(self, config):
        super(VideoTransformerAttPool, self).__init__()
        ...
        self.attention_pool = AttentionPool(self.transformer_dims)
    ...
    def forward(self,
        ...
        if self.transformer_feed_spatial:
            input_vec = torch.cat([spat, temp], 1).permute(1,0,2)

            device = input_vec.device
            tf_pad_mask = torch.cat([
                            torch.zeros((B, input_vec.shape[0] - frames_pad_mask.shape[1]),dtype=torch.bool).to(device),
                            frames_pad_mask,
                        ], 1)
            assert tf_pad_mask.shape == (input_vec.shape[1], input_vec.shape[0])
        else:
            input_vec = temp.permute(1,0,2)
            tf_pad_mask = frames_pad_mask

        # Shape of `output_vectors` (BS, T, D)
        output_vectors = self.transformer_encoder(src=input_vec, src_key_padding_mask=tf_pad_mask).permute(1,0,2)

        # Attention Pool
        output_vectors = self.attention_pool(output_vectors, tf_pad_mask)

        # Tagger layer 
        return self.output_tagger(output_vectors)
