...
from common.utils.matcher import HungarianMatcher
...
class ActionClassificationLoss(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        loss = self.ce_loss(outputs, targets['labels'])
        ...
        # Outputs
        outputs = {'loss':(loss.item(), 'Loss'), 'accuracy':(accuracy, 'Accuracy')}
...
class DetrLoss(nn.Module):
    ...
    """
    def __init__(self, config, isdist, world_size):
    ...
    def forward(self, outputs, indices, targets,):
        ...
        num_boxes = torch.clamp(num_boxes / self.world_size, min=1).item()
